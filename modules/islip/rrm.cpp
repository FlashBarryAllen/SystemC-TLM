...
rrm::rrm(int input_num, int output_num, int iterations, int voq_size)
{
...
void rrm::init_priority_ptr() {
    for (int i = 0; i < m_output_num; i++) {
        m_g_ptr[i] = (i + 1) % m_output_num;
    }
}

void rrm::request(int input, int output)
{
...
vector<pair<int, int>> rrm::get_sch_result()
{
    return sch_result;
}
