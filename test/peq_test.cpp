...
{
  #ifdef DEBUG
    fout << sc_time_stamp() << ", ----------------------------- Called allocate(), #trans = " << ++count << endl;
  #endif
  gp_t* ptr;
  if (free_list)
  {
...
{
  #ifdef DEBUG
    fout << sc_time_stamp() << ", ----------------------------- Called free(), #trans = " << --count << endl;
  #endif
  if (!empties)
  {
    empties = new access;
...
struct Initiator: sc_module
{
  ...
  void thread_process()
  {
    ...
    for (int i = 0; i < 1000; i++)
    {
      ...
      fout << sc_time_stamp() << ", addr=" << hex << adr << " new, cmd=" << (cmd ? 'W' : 'R')
           << ", data=" << hex << data[i % 16] <<  endl;
    ...
    fout << sc_time_stamp() << ", Calling b_transport with delay = " << delay << endl;
  ...
  void peq_cb(tlm::tlm_generic_payload& trans, const tlm::tlm_phase& phase)
  {
      ...
      if (phase == tlm::END_REQ)
        fout << sc_time_stamp() << ", addr=" << hex << trans.get_address() << " END_REQ at " << sc_time_stamp() << endl;
      else if (phase == tlm::BEGIN_RESP)
        fout << sc_time_stamp() << ", addr=" << hex << trans.get_address() << " BEGIN_RESP at " << sc_time_stamp() << endl;
    #endif
  ...
  void check_transaction(tlm::tlm_generic_payload& trans)
  {
    ...
    fout << sc_time_stamp() << ", addr=" << hex << adr << " check, cmd=" << (cmd ? 'W' : 'R')
...
struct Target: sc_module
{
  ...
  void peq_cb(tlm::tlm_generic_payload& trans, const tlm::tlm_phase& phase)
  {
    ...
    case tlm::BEGIN_REQ:
      ...
      #ifdef DEBUG
        fout << sc_time_stamp() << ", addr=" << hex << trans.get_address() << " BEGIN_REQ at " << sc_time_stamp() << endl;
      #endif
    ...
    case tlm::END_RESP:
      ...
      #ifdef DEBUG
        fout << sc_time_stamp() << ", addr=" << hex << trans.get_address() << " END_RESP at " << sc_time_stamp() << endl;
      #endif
      ...
      if (phase == internal_ph)
      {
        ...
        if ( cmd == tlm::TLM_READ_COMMAND )
        {
          *reinterpret_cast<int*>(ptr) = rand();
          fout << sc_time_stamp() << ", addr=" << hex << adr << " Execute READ, data = " << *reinterpret_cast<int*>(ptr) << endl;
        }
        else if ( cmd == tlm::TLM_WRITE_COMMAND )
          fout << sc_time_stamp() << ", addr=" << hex << adr << " Execute WRITE, data = " << *reinterpret_cast<int*>(ptr) << endl;
